...
use crate::envelope::{Datagram, Envelope, Protocol};
use crate::host::is_same;
use crate::kernel::core::socket::{Socket, SocketProtocol};
use crate::{ToSocketAddrs, World, TRACING_TARGET};
...
impl UdpSocket {
    pub(crate) fn new(local_addr: SocketAddr, socket: Socket) -> Self {
        Self { local_addr, socket }
    }
    ...
    pub async fn bind<A: ToSocketAddrs>(addr: A) -> Result<UdpSocket> {
        World::current(|world| {
            ...
            let sock = host.kernel.bind(addr, SocketProtocol::Udp)?;

            Ok(UdpSocket::new(addr, sock))
        })
    }
    ...
    pub async fn send_to<A: ToSocketAddrs>(&self, buf: &[u8], target: A) -> Result<usize> {
        World::current(|world| {
            ...
            host.kernel
                .send_to(Datagram(Bytes::copy_from_slice(buf)), dst)?;
            Ok(buf.len())
        })
    }
    ...
    fn drop(&mut self) {
        World::current_if_set(|world| {
            world
                .current_host_mut()
                .kernel
                .unbind(&mut self.socket)
                .expect("failed to unbind socket")
        });
    }
}
...
//     pub async fn bind<A: ToSocketAddrs>(addr: A) -> Result<UdpSocket> {
//         let (mut host, mut addr) =
//             World::current(|world| (world.current_host_mut(), addr.to_socket_addr(&world.dns)));
//
...
//         }
//
//         let sk = host.kernel.bind(addr, SocketType::Udp)?;
//         Ok(UdpSocket::new(addr, sk))
//     }
//
...
//     pub async fn send_to<A: ToSocketAddrs>(&self, buf: &[u8], target: A) -> Result<usize> {
//         let dst = World::current(|world| target.to_socket_addr(&world.dns));
//
//         self.send(world, dst, Datagram(Bytes::copy_from_slice(buf)))?;
//         Ok(buf.len())
//     }
